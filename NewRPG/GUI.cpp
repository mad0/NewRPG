...
//Default constructor
styleGUI::styleGUI(const std::string& _buttonText, sf::Font& _buttonFont, sf::Vector2f _buttonPosition) : buttonText(_buttonText), buttonFont(_buttonFont), buttonPosition(_buttonPosition) {
...
//Constructor
styleGUI::styleGUI(const std::string& _buttonText, sf::Font& _buttonFont, sf::Vector2f _buttonPosition, sf::Color _textColor, int _size) : buttonText(_buttonText), buttonFont(_buttonFont), buttonPosition(_buttonPosition), textColor(_textColor), size(_size){
	std::cout << "START TEXTGUI...\n";
	buttonSprite.setCharacterSize(size);
	buttonSprite.setString(this->buttonText);
	buttonSprite.setFont(this->buttonFont);
...
sf::Vector2f styleGUI::getPosition(){ return buttonSprite.getPosition(); }

//Set text position
void styleGUI::setPosition(sf::Vector2f _newPos) { buttonSprite.setPosition(_newPos); }

//Get text bounds
sf::FloatRect styleGUI::getBounds() { return buttonSprite.getGlobalBounds(); }

//Set text
void styleGUI::setText(const std::string & _text) { buttonSprite.setString(_text); }

//Destructor
styleGUI::~styleGUI(){ std::cout << "END TEXTGUI...\n"; }

//Draw buttonSprite
void styleGUI::Draw(sf::RenderTarget& target) { target.draw(buttonSprite); }

void styleGUI::Update() {
	buttonSprite.setString(this->buttonText);
}
